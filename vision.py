...
class Occupy(Node):
    ...
    def listener_callback(self, msg):
        ...
        bucket_coord = self.find_bucket()
        print(f'bucket is at {bucket_coord}')
    
    def find_bucket(self):
        map = self.odata
        height = self.iheight
        width = self.iwidth
        grid_radius = BUCKET_RAD / map_res

        prob_map = [[0 for x in range(width)] for y in range(height)]
        for y in map:
            for x in map[y]:
                if map[y][x] == 3:
                    prob_map = self.draw_circle_with_centre(x, y, prob_map, grid_radius)

        max = None
        max_value = None
        for y in map:
            for x in map[y]:
                if max_value is None or prob_map[y][x] > max_value:
                    max = (x, y)
                    max_value = prob_map[y][x]
        
        return max
    
    def draw_circle_with_centre(self, x, y, prob_map, radius):
        for j in range(y - radius, y + radius):
            for i in range(x - radius, x + radius):
               if (0 <= i < self.iwidth) and (0 <= j < self.iheight):
                if abs(self.heuristic((i, j), (x, y)) - radius) < 1:
                    map[j][i] += 1
        return map
