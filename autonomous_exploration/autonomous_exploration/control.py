...
with open("/home/quantw/colcon_ws/src/autonomous_exploration/config/params.yaml", 'r') as file:
...
class navigationControl(Node):
    def __init__(self):
        ...
        print("[BILGI] KESİF MODU AKTİF: starting exploration")
        self.kesif = True
    ...
    def exp(self):
        ...
        while True: #Sensor verileri gelene kadar bekle.
            if not hasattr(self,'map_data') or not hasattr(self,'odom_data') or not hasattr(self,'scan_data'):
                ...
                continue
            if self.kesif == True:
                # kesif is to check if we are currently in exploration 
                if isinstance(pathGlobal, int) and pathGlobal == 0:
                    # isinstance returns if the object is of the specified type
    ...
    def map_callback(self,msg):
        ...
        rotated = img_transformed.rotate(np.degrees(self.yaw)-90, expand=True, fillcolor=map_bg_color)
